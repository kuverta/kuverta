...
Written 2026-09-11 in the `fuckmail` repository; carried across the same day as
this repository's first document.

Kept as written. Two things have changed since, and rather than edit the
reasoning to match, both are marked below with **Since:** and recorded in
[decisions.md](decisions.md). Links that pointed inside `fuckmail` now name the
repository instead of pretending to resolve.
...
- `fuckmail/readme.md` — the design decisions section in particular
- `fuckmail/docs/implementation-plan.md` — §1a on the two
  stages of write capability, §4 on the classifier, §5 on risks
- `fuckmail/crates/core-rules/` — the classifier, portable as-is
- `fuckmail/crates/core-proto/src/mutate.rs` — the
  conflict checks of §3.5, with the reasoning in the comments
- `fuckmail/docker/fill-mailbox.py` — fills a test mailbox
