/**
 * The `kuverta` adapter against the host contract.
 *
...
import { KuvertaHost } from '../hosts/kuverta/host.js';
...
runConformance('kuverta', () => new KuvertaHost(fakeInvoke(), account).open());

// -- things specific to this host ---------------------------------------

test('kuverta: undo is queued, which is a different promise from compensating', async () => {
  const host = await new KuvertaHost(fakeInvoke(), account).open();
  assert.equal(host.capabilities.undo, 'queued');
});

test('kuverta: filing a message is not queued, so undo does not reverse it', async () => {
...
test('kuverta: a category that is not one is refused by the store, not the adapter', async () => {
...
test('kuverta: an account with no Archive folder cannot archive', async () => {
  const invoke = fakeInvoke();
  const bare = async (command, args) =>
    command === 'special_folders' ? { archive: null, trash: 'Trash' } : invoke(command, args);

  const host = await new KuvertaHost(bare, account).open();
  assert.equal(host.capabilities.archive, false);
  assert.equal(host.capabilities.trash, true);
});

test('kuverta: dates cross the bridge in seconds and arrive in milliseconds', async () => {
...
test('kuverta: a refused change is called out in the sync line', async () => {
  ...
  const host = await new KuvertaHost(withRefusal, account).open();
...
test('kuverta: an unknown command is an error, not a silent null', async () => {
...
test('kuverta: post appears in the same list as mail, in date order', async () => {
...
test('kuverta: a document reads like a message', async () => {
  const host = await new KuvertaHost(fakeInvoke(), account).open();
  const { rows } = await host.page({ scope: { kind: 'all' }, offset: 0, limit: 50 });
...
test('kuverta: an address can be read on its own', async () => {
  const host = await new KuvertaHost(fakeInvoke(), account).open();
...
test('kuverta: a category scope takes in post as well as mail', async () => {
...
test('kuverta: acting on post is refused, except filing it', async () => {
...
test('kuverta: post filed by hand shows where it was filed', async () => {
  const host = await new KuvertaHost(fakeInvoke(), account).open();
  const { rows } = await host.page({ scope: { kind: 'all' }, offset: 0, limit: 50 });
  const letter = rows.find((row) => row.id.startsWith('paper:'));
...
test('kuverta: the triage surface files a letter instead of refusing it', async () => {
...
test('kuverta: a Paperless that is down does not take the mail with it', async () => {
  ...
  const host = await new KuvertaHost(broken, account).open();
...
test('kuverta: an address with no token is not offered', async () => {
  // A scope that can only ever fail is worse than one that is not there.
  const paper = { paperMailboxes: [], documents: [] };
  const host = await new KuvertaHost(fakeInvoke({ paper }), account).open();
...
test('kuverta: an opened message carries the facts its verdict is explained from', async () => {
...
test('kuverta: an opened letter is explained the same way as mail', async () => {
  const host = await new KuvertaHost(fakeInvoke(), account).open();
  const { rows } = await host.page({ scope: { kind: 'all' }, offset: 0, limit: 50 });
  const letter = rows.find((row) => row.id.startsWith('paper:'));
