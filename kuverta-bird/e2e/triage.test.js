 ...
 * off screen. These tests load the page `kuverta` actually mounts —
 * `hosts/kuverta/ui/triage.html`, over HTTP, since ES modules will not load
...
const PAGE = '/hosts/kuverta/ui/triage.html';
...
async function open({ count = 0, paper = false, seen = true, width = 1200, height = 800 } = {}) {
  ...
  await page.addInitScript(
    ({ count, paper, seen }) => {
      if (seen) localStorage.setItem('kuverta-bird.explained', '1');
      let bridge;
      window.__TAURI__ = {
        core: {
