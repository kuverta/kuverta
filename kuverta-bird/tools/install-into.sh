#!/usr/bin/env bash
#
# Copy the shared core and the kuverta adapter into that app's UI directory.
#
...
#
#   <target>/kuverta-bird/core/...
#   <target>/kuverta-bird/hosts/kuverta/...
#
...
if [[ -z $TARGET ]]; then
    ...
    echo "   eg: kuverta-bird/tools/install-into.sh apps/desktop/ui" >&2
    exit 1
fi
...
cp -R "$SOURCE/hosts/kuverta" "$DEST/hosts/kuverta"

# The copy is only worth anything if it is actually identical.
diff -r "$SOURCE/core" "$DEST/core" >/dev/null
diff -r "$SOURCE/hosts/kuverta" "$DEST/hosts/kuverta" >/dev/null

FILES=$(find "$DEST" -type f | wc -l | tr -d ' ')
echo "installed $FILES files into $DEST"
echo "open it at: kuverta-bird/hosts/kuverta/ui/triage.html"
